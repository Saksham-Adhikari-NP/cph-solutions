...
int32_t main() {
   ...
   // freopen("input.txt", "r", stdin);   // debug
    ...
    while (t--) {
        ...
        int n , m ; cin >> n >> m ; 
        vector<int> k(n) , c(m) ; cin >> k >> c ; 
        vector<int> srtd_gft = c ; 
        sort(all(srtd_gft)) ;

        sort(all(k) , [&] (int &r , int &l) { return c[r-1] > c[l-1] ; } ) ; // sort them acc to hoe exp cash would be   ;

        int cnt = 0 , ans = 0 ; 
        FOR(i,0,n) 
        {
            if (srtd_gft[cnt] < c[k[i] - 1] && cnt < m) 
            {
                ans += srtd_gft[cnt] ;
                cnt++ ;
            } 
