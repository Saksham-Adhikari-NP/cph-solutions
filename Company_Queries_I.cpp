...
// preprocessing for pow of two ; 
    ...
    function<int (int,int)> kth_parent =[&] (int node,int k) 
    {   
        FOR(bit , 0, 31) 
        {
            if((1 << bit) & k)
            {
                if(node!= -1) 
                {
                    node = up[node][bit] ;  // cause we already preprocess in power of 2 ;;; 
                }
            }
        }
        return node ; 
