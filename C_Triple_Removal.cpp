...
void solve () 
{   // for any adjacent pair with  common pair l it can be dissolved into op 1 ;; 100101 ;; 00-0 - 111 ; 
    // if only adjacent 1 operation to make it a common pair + adjacent  
    // solve here
    
    int n , q ; cin >> n >> q ; 
    vi a(n) ; cin >> a ; 
    vector<vi> pre(2,vi (n+1)) ;
    FOR(x,0,2) 
    {
        FOR(i,0,n) 
        {
            pre[x][i+1] = pre[x][i] + (a[i] == x) ;
        }
    }
    vi d(n) ;
    ...
    while (q--) 
    {
        ...
        cin >> l >> r ; 
        l-- ;
        int n0 = pre[0][r] - pre[0][l] , n1 = pre[1][r]  - pre[1][l] ;
        if ( n1%3 || n0%3) 
        {
             cout << -1 << endl;
        }
        else  cout << (r-l)/3 + (d[r-1] == d[l]) << endl; 
    }
}
