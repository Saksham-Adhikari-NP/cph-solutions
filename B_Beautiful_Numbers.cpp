...
void solve () 
{
    ...
    FOR(i, 0, n) {
        ...
        cur_sum += d;
        if (i == 0) {
            // Leading digit: cannot be 0, so min value is 1.
            // Max reduction possible is (d - 1).
            reductions.pb(d - 1);
        } else {
            // Other digits: can be 0.
            // Max reduction possible is d.
            reductions.pb(d);
        }
    }
    if (cur_sum <= 9) {
    ...
    int target_reduction = cur_sum - 9;
    sort(rall(reductions));

    int moves = 0;
    int total_red = 0;
    trav(r, reductions) {
        if (total_red >= target_reduction) break;
        total_red += r;
        moves++;
    }
