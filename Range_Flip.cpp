...
long long max_sum_k_subarrays(const vector<int>& arr, int k) {
    int n = arr.size();
    if (n == 0 || k == 0) return 0;

    vi prev_global(n + 1, 0);
    
    for (int i = 1; i <= k; ++i) {
        vi curr_global(n + 1, 0);
        int local_max = LLONG_MIN;
        
        for (int j = 1; j <= n; ++j) {
            local_max = max(local_max, prev_global[j - 1]) + arr[j - 1];
            curr_global[j] = max(curr_global[j - 1], local_max);
        }
        prev_global = curr_global;
    }

    return prev_global[n];
}
...
void solve () 
{
    ...
    int n , k ; 
    cin >> n >> k ;
    vi front(n) ; vi back(n) ; 
    for(int i = 0 ; i<n ; i++) 
    {
        cin >>front[i] >> back[i] ; 
    }
    vi diff(n) ; 
    FOR(i,0,n) diff[i] = back[i] - front[i] ; 

    cout << accumulate(all(front) , 0ll) + max_sum_k_subarrays(diff , k) << endl ; 
