...
void solve () 
{    // o - e  == e - o ;;; o - e + (2e - 2o) + ( r- l) :: 2e + r - )(2o + l ) ;   :: 
    // solve here
    int n ; cin >> n ; 
    ...
    vi  maxevenr(n+2), minoddl(n+1);
    vi maxoddr(n+2, -INF), minevenl(n+1, INF);   
    minoddl[0] = INF ; maxevenr[n+1] = -INF ; 

    FOR(i,1,n+1) { (i&1) ? minoddl[i] = min(minoddl[i-1] , 2*a[i] + i ) : minoddl[i] = minoddl[i-1] ;}
    FORd(i,1,n+1) {  (!(i&1)) ? maxevenr[i] = max(maxevenr[i+1] , 2*a[i] + i ) : maxevenr[i] = maxevenr[i+1] ; } 

    FOR(i,1,n+1) minevenl[i] = (!(i & 1)) ? min(minevenl[i-1], i - 2*a[i]) : minevenl[i-1];
    FORd(i,1,n+1) maxoddr[i] = (i & 1) ? max(maxoddr[i+1], i - 2*a[i]) : maxoddr[i+1];
 
    int ans =  fn  ;
    FOR(i,1,n+1) {
        if(i & 1) ans = max(ans, fn + (i - 1)); 
        else if (i > 1) ans = max(ans, fn + (i - 2));
    }
    FOR(i,1,n+1) {  ans  = max({ans  , fn + maxevenr[i]  - minoddl[i] , fn + maxoddr[i] - minevenl[i] }) ; }
    cout << ans << endl ; 
}
