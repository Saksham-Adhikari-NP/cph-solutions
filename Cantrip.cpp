...
void solve () 
{
    ...
    int n ; cin >> n ;
    string s ; cin >> s ;
    vi pref(n,0),cost(n,0) ;
    if(s[0] == 'o') pref[0] ++ ; 
    else cost[0] ++ ; 
    for(int i = 1; i<n ; i++) 
    {
        pref[i] = pref[i-1] + (s[i] == 'o' ? 1 :0 ) ; 
        cost[i] = cost[i-1] + (s[i] == 'x' ? 1:0) ; 
    }
    vi ans ; 
    for(int i = 0 ; i<n ; i++) 
    {

            if(pref[i] == 0 ) { ans.push_back(i+1) ; continue ; }

        int CAN  = cost[i] + pref[i];  
        auto it = lower_bound(all(cost) , CAN) ;

        if(it == cost.end()) ans.push_back(n) ;

        else {
            int j = distance(cost.begin() , it) ; 
            ans.push_back(j+1) ; 
        }
