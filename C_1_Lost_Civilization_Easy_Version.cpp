...
void solve () 
{
    // solve here
        ...
        // For each left endpoint l, run the greedy on subsegments starting at l
        for (int l = 0; l < n; ++l) {
            // s stores current chain-ends in non-decreasing order (we keep it small,
            // following the same logic you had). For each new x we try to find
            // a chain with end == x-1; if none, we start a new chain (ans++).
            vector<long long> s;
            long long ans = 0;
            for (int r = l; r < n; ++r) {
                long long x = a[r];
                // pop any chain-ends greater than x-1
                while (!s.empty() && s.back() > x - 1) s.pop_back();

                if (!s.empty() && s.back() == x - 1) {
                    s.push_back(x);
                } else {
                    // need a new initial element for this subsegment extension
                    ++ans;
                    s.clear();
                    s.push_back(x);
                }
