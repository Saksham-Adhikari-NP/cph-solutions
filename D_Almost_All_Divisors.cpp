...
void solve () 
{
    ...
    for(int i = 2 ; i <= sqrt(cd) ; i++)
    {
         if(cd % i == 0 ) 
         { candidate.pb(i);
             if(i*i != cd)  candidate.pb(cd/i) ; }
         }
    sort(all(candidate)) ;
    if (candidate == div) cout << cd << endl ; 
    else cout <<-1 << endl ;
    

//     long long cd = 1LL * div.front() * div.back();
// vector<long long> candidate;

// for(long long i = 2; i * i <= cd; i++){
//     if(cd % i == 0){
//         candidate.push_back(i);
//         if(i * i != cd)
//             candidate.push_back(cd / i);
//     }
// }
