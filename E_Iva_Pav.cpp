...
vector<vector<int>> idx_boc (2e5 ,vector<int>(31)) ; 

int bs( int l , int r , int k ) {
    int ans = 0 ; 
    FOR(i,0,31) 
    {
        if((idx_boc[r][i] - idx_boc[l-1][i] ) == r-l+1  ) ans += 1ll << i ; 
...
void solve () 
{
    // solve here
    int n ; cin >> n ;
    vector<int> arr(n+1) ; FOR(i,1,n+1) cin >> arr[i]  ; 
    FOR(i,1,n+1) 
    {
        FOR(j,0,31) {
            idx_boc[i][j]  = idx_boc[i-1][j]  + ((arr[i] >> j) & 1) ; 
        }
    }
    int q , l, k ;
    cin >> q ;
    while ( q--) 
    {
         ...
         while(low <= high ) {
            ...
            if( bs(l,mid, k))  { ans = mid ; low = mid + 1; } 
            else high = mid-1 ; 
