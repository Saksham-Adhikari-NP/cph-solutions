...
void solve () 
{
    ...
    {
        if(a[i] == '1') {
            if(i&1) oda.push_back(i/2) ; 
            else eva.push_back(i/2) ; 
        }

         if(b[i] == '1') {
            if(i&1) odb.push_back(i/2) ; 
           else evb.push_back(i/2) ; 
             
        }


    }
    if(  (sz(oda)  != sz(odb))  || ( sz(eva)  != sz(evb) ) ) { cout << - 1 << "\n" ;   return ; } 

    int ans = 0; 
    
    for(int i = 0 ; i<sz(eva) ; i++) ans += abs(eva[i] - evb[i]) ;
    for(int i = 0 ; i<sz(oda) ; i++) 
    {
        ans += abs(oda[i]-odb[i]) ; 
    }
