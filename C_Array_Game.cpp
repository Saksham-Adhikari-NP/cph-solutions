...
void solve () 
{
    // solve here
             ...
             // for 2 steps u need like a diff thats like eqv to sm arr [elem ] or has the smallest diff with the arr 
         // elem so u could do that for the next step ;; or even after that smalelst could still
         // be like 1st elem ;; 
         // smallest diff  could be  > smallest elem   

    int n , k ; 
    cin >> n >> k ;
    vector<int> arr(n) ; cin >> arr  ; sort(all(arr)) ; 
    if (k>= 3) 
    {
        cout << 0 << endl ;
        return ;
        ...
        int ans = arr[0]  ; 
        FOR(i,1,n) ans = min (ans  , abs(arr[i] - arr[i-1] )) ;
            if ( k == 1) 
    {
    ...
    if (k==2) 
    {
         FOR(i,0,n) 
         {
            FOR (j,0, i)   
            {
                ...
                auto ldar = lb(all(arr) , curr) ;

                if (ldar != arr.end())  ans = min(ans, abs(*ldar - curr));
                 if (ldar != arr.begin())  ans = min(ans, abs(*(ldar- 1) - curr));
