...
{
    // solve here
    WeightedGraph adj(MAXN) ; 
    int nodes , routes  ; 
    cin >> nodes >> routes ; 
    vi distance(nodes+1 , INF);
    vi processed(nodes+1 , 0) ; 
    while (routes--) 
    {
        int a,b,c ;  cin >> a >> b >> c ; 
        adj[a].pb({b,c}) ; 
    ...
    priority_queue<pair<int,int>> q ; // weight , edge ' 
    distance[1] = 0 ; 
    ...
    FOR(i,1,nodes+1) cout << distance[i] << " "; 
    cout << endl ; 
