...
{
    // solve here
    // process 0-n ;; if the active no of segments > k ;; remove the curr active seg that reaches the farthest and store seg,s idx ; 
    // 
    int n , k ;  cin >> n >> k ; 
    vector<pair<int,pii> > v(n)  ; // l , r, i 
    FOR(i,0,n) { cin >> v[i].first >> v[i].second.first ;  v[i].second.second = i ; }
    sort(all(v)) ; 
    vector<int> rmvd;
    set<pair<int,int>> act_seg;
    FOR(i,0,n){
        // remove rs less than the curr i 

        while (!act_seg.empty() && act_seg.begin()->first < v[i].first) { 
            act_seg.erase(act_seg.begin());
        }

        act_seg.insert(v[i].second);

        if ((act_seg.size()) > k) {
            rmvd.pb((act_seg.rbegin()->second)+1); 
            act_seg.erase(--act_seg.end());
        }
    }
    cout << rmvd.size() << endl;
cout << rmvd; 
