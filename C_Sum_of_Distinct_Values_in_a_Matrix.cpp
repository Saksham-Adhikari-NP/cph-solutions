...
void solve() 
{
    ...
    map<int,int , greater<int>  > desctype  ; 
    for(auto x:a) desctype[x] |= 1 ; 
    for(auto x:b) desctype[x] |= 2 ; 

    auto check = [&] (int rw  ,int cl)  -> int {
        int us1 = 0 , us2 = 0  , totuse = 0  , sum = 0 ; 

        for(auto[val,type] : desctype ) 
        {
            if(totuse == rw+cl) break ; 
            if(type == 3) {
                totuse ++ ; sum += val ; 
            }
            else if(type == 1 && us1 < rw) {
                us1 ++ ; sum += val ; totuse++ ;  
            }
            else if(type == 2 && us2 < cl) {
                us2 ++ ; sum += val ; totuse++ ; 
            }
    ...
    int rwbnd = min(n,x) ; 
    int clbnd = min(m,y) ; 
    int ans = ((rwbnd == n && clbnd == m) ? max( check(rwbnd-1, clbnd), check(rwbnd,clbnd-1) ) : check(rwbnd , clbnd)) ; 
