...
{
    // solve here
    // increment occurs at the first index where x is bigger then the prev elem 
    // x must never be bigger then the min pref till that idx 
    // to increment in a idx x must be bigger than that idx ; 
    // so max we could do is like ;;
    // m-1 first , then m second ; so max is like 2 m - 1 ; 
    // is goal is bigger than that ans no  
    int n ; cin >> n ; 
    vi b(n) ; cin >> b ; 
    int mn = b[0] ;
    trav(x,b) 
