...
// n == 1  d[0] , d[0]d[0] , d[0]d[0]d[0] 
// n == 10 ; all dig = 0 ;
// else either  ;; sz.ans =   n ; or n-1 ; or n+1 ;;;  
    if (n == 1) {
        if (d[0] == 0) 
        {
            cout << a << '\n';
    ...
    for (int l = max(1ll, s - 1); l <= s + 1; l++) 
    {
        ...
        int low = 0, high = modpow(n, l) - 1; // -> n^l -> 9^18 ~= 1e18 
        int y = low, z = high;
        ...
        ans = min({ans, abs(a - f(y)) , abs(a - f(z))});
    }
    cout << ans << endl ; 
