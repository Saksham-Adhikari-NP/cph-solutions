...
void solve () 
{
    ...
    reach.reserve(n * 60); // log2 10^9 - around 30 ;; max 40 itr for each num ; 
    ...
    int ops = INF;
    for (int i = 0; i < sz(reach); ) {
        int j = i;
        int curr = reach[i].first;
        int current_sum = 0;
        
        // Group all instances of current_val
        while (j < sz(reach) && reach[j].first == curr) {
            current_sum += reach[j].second;
            j++;
        }
        
        // If the number of instances equals n, all original numbers can reach this value
        if (j - i == n) {
            ops = min(ops, current_sum);
        }
