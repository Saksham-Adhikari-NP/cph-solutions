...
void solve () 
{
    ...
    vector<pii> b;
    for (int x : a) {
        if (b.empty() || b.back().first != x) b.push_back({x, 1});
        else b.back().second++;
    }
    ...
    auto safe = [&](int i, int j) {
        return j < 0 || j >= k || b[i].first != b[j].first;
    };

    FOR(i, 0, k) {
        if (b[i].second < 2) continue;

        if (i + 1 < k && b[i + 1].second >= 2) gain = max(gain, 2LL);
        if (i + 1 < k && safe(i, i + 2)) gain = max(gain, 1LL);
        if (i - 1 >= 0 && safe(i, i - 2)) gain = max(gain, 1LL);
    }
