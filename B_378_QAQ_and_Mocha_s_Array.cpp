...
int32_t main() {
    ...
    while (t--) {
        ...
        int  n ;
        cin >> n ;
        vector<int> arr (n) , cross (n,1)  ;
        cin >> arr ; 
        sort(all(arr) ) ; 
        int not_div  ;
        bool switche = false ; 
        auto div_rem = [&arr , & cross , &not_div , &switche , &n] (int p ) 
        {
            for ( int i = p ; i < n ; i++ ) 
            {
                if (arr[i] % arr[p] == 0 && cross[i] != 0 ) cross[i] = 0 ;
                if ( arr [i] % arr[p] != 0 && !switche ) 
                {
                    not_div = i ;
                    switche = true ; 
                } 
        ...
        div_rem(0) ; 
        div_rem(not_div) ;
        int sum = accumulate(all(cross) , 0ll) ;
