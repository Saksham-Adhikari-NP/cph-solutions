...
const int primes_till = 1e6 * 6;
vector<int> primes ; 
vector<bool> is_prime(primes_till+1,true) ;  
void sieve () 
{
    is_prime[0] = false ; is_prime[1] = false ; 
    FOR(i,2,primes_till+1) 
    {
        if(is_prime[i]) 
        {
            primes.pb(i) ;
            if (i*i <= primes_till ) 
            {
                for(int j = i*i ; j<= primes_till ; j+= i ) 
                {
                    is_prime[j] = false ; 
                }
            } 
...
void solve () 
{
    ...
    cin >> arr ; 
    sort(rall(arr)) ;
    vector<int> prefxarr (n) ; prefxarr[0] = arr[0] ; 
    FOR(i,1,n) prefxarr[i] = prefxarr[i-1] + arr[i] ; 
    int limit = min((int)primes.size(), n);
    int k = 0;

    while (k < limit && prefp[k] <= prefxarr[k]) {
        k++;
    }
